...
async function handlePushToClick() {
  ...
  try {
    const [astapStatus, catalogStatus] = await Promise.all([
      getAstapStatus(),
      getCatalogStatus(),
    ])
...
watch(() => eventStream.lastEvent.value, (event) => {
...
<template>
  ...
  <div v-else class="app">
    ...
    <header class="header">
      ...
      >
        <svg v-if="!checkingSetup" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
