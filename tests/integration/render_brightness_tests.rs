...
use crate::integration::stack_depth_grain_tests::{
    managed_session, render, render_with, stack_snapshots,
};
...
fn a_bright_star_keeps_no_ring() {
    ...
    // Every configuration that denoises: the default, the top of the Background Grain
    // dial where wavelet levels 5-6 run, and Star Fields, which is the only profile that
    // *amplifies* the fine scales. All three have a history here — the coarse levels move
    // a star's flux outward into a disc if thresholded carelessly, and the Star Fields
    // gain digs a moat around every star if pushed past ~1.4.
...
/// The Background Grain dial, end to end, on all four sessions.
///
/// What this is for: each step must be monotone in **both** brightness and grain, the
/// star radial profile must keep its shape at every position, and the middle must
/// reproduce what shipped before the dial existed. A dial with a non-monotone middle
/// would be worse than the three independent controls it replaced.
///
/// Note which half does what. Below the middle the dial moves fine speckle and the tone
/// curve; above it the coarse wavelet levels run, and only those reach the 8-128 px bands
/// an observer actually reads grain in.
#[test]
#[serial]
#[ignore = "diagnostic - the four sessions live outside the repo; run with --ignored"]
fn sweep_the_background_grain_dial() {
    println!("\noctaves: {}", OCTAVE_LABELS.join("  "));
    for (name, dir) in requested_sessions() {
        let Some((stack, depth)) = cached_stack(name, dir) else {
            println!("  {name}: not on this machine, skipped");
            continue;
        ...
        let a = anchors(name, &stack, depth);
        println!("        {}", profile_header());
        for dial in [0.0f32, 0.25, 0.5, 0.75, 1.0] {
            let mut settings = night_amplifier::server::state::CaptureSettings::default();
            settings.denoise = night_amplifier::server::state::DenoiseSettings {
                background_grain: dial,
                ..Default::default()
            };
            let split = settings.denoise.grain_split();
            let protection = settings.denoise.star_protection();
            let (rgb8, w, h) = render(stack.clone(), &settings, true, stream(), depth);
            report(
                &format!("{name} {:>3.0}% p{protection:.2} s{split:.3}", dial * 100.0),
                &rgb8,
                w,
                h,
                &a,
...
// ---------------------------------------------------------------------------
// The observer's own stacked sessions
// ---------------------------------------------------------------------------

/// Stacks saved by the running application, which is what the observer actually looked
/// at. Set `STACKED_DIR`; defaults to the Pro repo's capture directory.
///
/// These are f32 FITS, so `fits::read_frame` min/max-normalises them on load — harmless
/// *here* only because the writer emits an already-normalised frame (measured: min 0.0,
/// max 1.001-1.006, so the rescale is under 0.6 %). Do not reuse this loader for data
/// from anywhere else.
fn stacked_sessions() -> Vec<(String, u32, night_amplifier::Frame)> {
    let dir = std::env::var("STACKED_DIR").unwrap_or_else(|_| {
        "/opt/GitHub/night-amplifier-pro/captures/stacked/current".to_string()
    });
    ...
    let filter = std::env::var("STACKED_ONLY").ok();
    let mut out = Vec::new();
    for path in paths {
        let (label, depth) = stacked_header(&path);
        if let Some(list) = &filter {
            if !list.split(',').any(|w| label.contains(w.trim())) {
                continue;
            }
        }
        match night_amplifier::fits::read_frame(&path) {
            Ok(frame) => out.push((label, depth, frame)),
...
/// `INSTRUME` and `NCOMBINE` out of the FITS header, which carry what the stack is and
/// how deep it is. The depth is not bookkeeping: the tone curve is solved from it.
fn stacked_header(path: &Path) -> (String, u32) {
    let head = std::fs::read(path).unwrap_or_default();
    let head = String::from_utf8_lossy(&head[..head.len().min(16384)]).to_string();
    let value = |key: &str| -> Option<String> {
        let at = head.find(key)?;
        let card = &head[at..at + 80.min(head.len() - at)];
        let rest = card.split('=').nth(1)?;
        Some(rest.split('/').next()?.trim().trim_matches('\'').trim().to_string())
    };
    let label = value("INSTRUME")
        ...
        .unwrap_or("unknown")
        .to_string();
    let depth = value("NCOMBINE").and_then(|v| v.parse().ok()).unwrap_or(1);
    (label, depth)
}

/// Fine bands (1-8 px) and the bands the eye actually reads grain in (8-128 px), apart.
///
/// Keeping them apart is the whole point. A filter can halve the fine speckle and leave
/// the mottle an observer complains about untouched — which is exactly what
/// `star_protection` does, and why the dial's bottom half looked inert at the eyepiece
/// while every summary statistic said it was working.
fn split_bands(bands: &[f64; 7]) -> (f64, f64) {
    let rms = |xs: &[f64]| (xs.iter().map(|v| v * v).sum::<f64>() / xs.len() as f64).sqrt();
    (rms(&bands[0..3]), rms(&bands[3..7]))
}

/// Half-width of the region the octave bands are measured over.
///
...
/// 64-128 px numbers — the two the eye weighs most — an artefact of the box rather than
/// a property of the sky. 192 gives a 384 px region: three times the coarsest band.
const GRAIN_HALF_WIDTH: usize = 192;

/// The sky box grown about its own centre, clamped into the frame.
fn grain_box(a: &Anchors, w: usize, h: usize) -> (usize, usize, usize, usize) {
    let (cx, cy) = ((a.sky_box.0 + a.sky_box.2) / 2, (a.sky_box.1 + a.sky_box.3) / 2);
...
/// How many stars stand clear of the background, per megapixel.
///
/// The Star Fields profile is judged on this and not on target brightness: an open or
/// globular cluster is *made of* stars, so "can I see the field" is a count, not a core
/// level.
///
/// A 5x5 local maximum whose **3x3 mean** is at least `margin` output levels above the
/// frame's sky. The 3x3 mean is what makes the number mean anything: a bare peak test
/// counts single-pixel noise spikes as stars, and a variant that left the finest wavelet
/// scale untouched then scored 84 % *more* "stars" purely by keeping more speckle. A real
/// star at streaming resolution is 2-3 px across and carries its neighbours with it.
fn star_count(plane: &[f64], w: usize, h: usize, margin: f64) -> f64 {
    let sky = plane_median(plane);
    ...
    for y in 2..h - 2 {
        for x in 2..w - 2 {
            ...
            if best && block / 9.0 >= sky + margin {
                found += 1;
            }
        }
    }
    found as f64 / (w * h) as f64 * 1.0e6
}

/// Mean `|R-G| + |G-B|` over the 3x3 around each anchored star, in output levels.
///
/// The thing Asinh is in the Star Fields profile *for*: a curve that drives star cores to
/// white buys brightness by spending their colour, and a brightness-only metric cannot
/// see that happening.
fn star_chroma(rgb8: &[u8], w: usize, h: usize, a: &Anchors) -> f64 {
    let mut sum = 0.0;
    let mut n = 0.0;
    for star in &a.stars {
        for dy in -1i32..=1 {
            for dx in -1i32..=1 {
                ...
                let (r, g, b) = (rgb8[i] as f64, rgb8[i + 1] as f64, rgb8[i + 2] as f64);
                sum += (r - g).abs() + (g - b).abs();
                n += 1.0;
            }
        }
...
/// Share of samples the render drove to pure white, as a percentage.
///
/// The S-curve's other end: at `strength` 1.0 every stretched value above 0.610 clamps to
/// 1.0, against 0.668 at 0.8 — so raising it narrowed the highlight range by 9 %. Target
/// brightness measured at three radii cannot see that, because a core already at the
/// ceiling reads the same whether it is just at it or far past it.
fn white_share(rgb8: &[u8]) -> f64 {
    let n = rgb8.len() / 3;
    let clipped = rgb8.chunks_exact(3).filter(|p| p[0] == 255 && p[1] == 255 && p[2] == 255).count();
    100.0 * clipped as f64 / n.max(1) as f64
}

fn measure(rgb8: &[u8], w: usize, h: usize, a: &Anchors) -> (f64, ObjectExcess, f64, [f64; 7]) {
    let (plane, pw, ph) = green_region(rgb8, w, (0, 0, w, h));
    let (sky_plane, _, _) = green_region(rgb8, w, a.sky_box);
    let sky = plane_median(&sky_plane);
    let obj = object_excess(&plane, pw, ph, a.object, 96, sky);
    let star = radial_excess(&plane, pw, ph, &a.stars)[0];
    let (grain_plane, gw, gh) = green_region(rgb8, w, grain_box(a, w, h));
    (sky, obj, star, octave_band_sigma(&grain_plane, gw, gh))
}

fn header_line() {
    println!(
        "{:<26} {:>4} {:>6} {:>6} {:>6} {:>6} {:>7} | {:>5} {:>5} | {}",
        "", "sky", "core", "mid", "outer", "star", "white%", "fine", "vis",
        OCTAVE_LABELS.join("  ")
    );
}

#[allow(clippy::too_many_arguments)]
fn line(label: &str, sky: f64, obj: &ObjectExcess, star: f64, chroma: f64, bands: &[f64; 7]) {
    let (fine, visible) = split_bands(bands);
    println!(
        "{label:<26} {sky:>4.0} {:>6.0} {:>6.0} {:>6.0} {:>6.0} {chroma:>7.2} | {fine:>5.2} \
         {visible:>5.2} | {}",
        obj.core,
        obj.mid,
        obj.outer,
...
/// **Part 1.** What each Color mode does to the observer's own stacks.
///
/// `stretch_aggressiveness` picks the profile the UI calls Color mode, and each one sets
/// its own target background, black point and tone-mapping algorithm — but they all share
/// one `ContrastConfig`. This is the measurement for whether that is the right call.
#[test]
#[serial]
#[ignore = "diagnostic - reads saved stacks; run with --ignored"]
fn compare_colour_modes_on_saved_stacks() {
    use night_amplifier::render::StretchAggressiveness;
    use night_amplifier::server::state::{CaptureSettings, DenoiseSettings};

    let live = std::env::var("STACKED_LIVE_SETTINGS").is_ok();
    println!(
        "\ncontrast {:?}; denoise = {}",
        night_amplifier::render::ContrastConfig::default(),
        if live { "the observer's settings.json" } else { "product defaults" }
    );

    for (label, depth, stack) in stacked_sessions() {
        println!("\n=== {label}, N={depth} ===");
        let a = anchors(&format!("saved-{label}"), &stack, depth);
        header_line();
        for (name, mode) in [
            ("star-fields", StretchAggressiveness::Low),
            ("deep-sky", StretchAggressiveness::Medium),
            ...
            settings.stretch_aggressiveness = mode;
            settings.denoise = if live {
                DenoiseSettings {
                    chroma: false,
                    chroma_strength: 1.0,
                    background_grain: 0.2,
                    luma_strength: 0.2,
                }
            } else {
            ...
            if live {
                settings.auto_stretch_intensity = 0.15;
                settings.eyepiece.black_floor = -0.01;
            }
            let (rgb8, w, h) = render(stack.clone(), &settings, true, stream(), depth);
            let (sky, obj, star, bands) = measure(&rgb8, w, h, &a);
            let (plane, pw, ph) = green_region(&rgb8, w, (0, 0, w, h));
            line(name, sky, &obj, star, white_share(&rgb8), &bands);
            println!(
                "{:<26} stars/Mpx above sky+20: {:.0}   above sky+60: {:.0}",
                "",
                star_count(&plane, pw, ph, 20.0),
                star_count(&plane, pw, ph, 60.0)
            );
        }
    }
}

/// **Part 2.** The dial, against the bands the eye actually reads.
///
/// Swept at the shipped `luma_strength` and at the 0.2 the observer is running, because
/// that setting scales **every** wavelet threshold including the finest — so it scales
/// down whatever the dial is trying to do.
#[test]
#[serial]
#[ignore = "diagnostic - reads saved stacks; run with --ignored"]
fn sweep_the_dial_on_saved_stacks() {
    use night_amplifier::server::state::{CaptureSettings, DenoiseSettings};
    ...
    for (label, depth, stack) in stacked_sessions() {
        ...
        let a = anchors(&format!("saved-{label}"), &stack, depth);
        header_line();
        for luma_strength in [1.0f32, 0.2] {
            for dial in [0.0f32, 0.25, 0.5, 0.75, 1.0] {
                let mut settings = CaptureSettings::default();
                settings.denoise = DenoiseSettings {
                    chroma: false,
                    chroma_strength: 1.0,
                    background_grain: dial,
                    luma_strength,
                ...
                let (prot, split, coarse) =
                    (d.star_protection(), d.grain_split(), d.coarse_denoise());
                let (rgb8, w, h) = render(stack.clone(), &settings, true, stream(), depth);
                let (sky, obj, star, bands) = measure(&rgb8, w, h, &a);
                line(
                    &format!(
                        "ls{luma_strength:.1} d{:>3.0}% p{prot:.2} s{split:.3} c{coarse:.2}",
                        dial * 100.0
                    ),
                    sky,
                    &obj,
                    star,
                    star_chroma(&rgb8, w, h, &a),
                    &bands,
                );
            }
        }
...
/// **Part 2.** The dial's upper half, measured for both what it buys and the ringing the
/// coarse levels were rejected for the first time around.
///
/// The dial's lower half cannot reach 16-64 px, which is where an observer reads grain —
/// the transform used to stop at level 4 (~8-16 px). Levels 5-6 reach it, and they ring
/// without the interscale mask, so the brightness and grain columns mean nothing without
/// the profile underneath them.
#[test]
#[serial]
#[ignore = "diagnostic - reads saved stacks; run with --ignored"]
fn sweep_coarse_levels_on_saved_stacks() {
    use night_amplifier::server::state::{CaptureSettings, DenoiseSettings};
    ...
    for (label, depth, stack) in stacked_sessions() {
        ...
        let a = anchors(&format!("saved-{label}"), &stack, depth);
        header_line();
        println!("{:<26} {}", "", profile_header());

        for dial in [0.5f32, 0.75, 1.0] {
            let mut settings = CaptureSettings::default();
            settings.denoise = DenoiseSettings {
                chroma: false,
                chroma_strength: 1.0,
            ...
            // Warm first, then the median of five: a single shot on a loaded machine
            // moves further than the two extra wavelet levels do.
            let _ = render(stack.clone(), &settings, true, stream(), depth);
            let mut times = Vec::new();
            let mut last = None;
            for _ in 0..5 {
                let started = std::time::Instant::now();
                last = Some(render(stack.clone(), &settings, true, stream(), depth));
                times.push(started.elapsed());
            }
            times.sort();
            let elapsed = times[times.len() / 2];
            let (rgb8, w, h) = last.unwrap();

            let (sky, obj, star, bands) = measure(&rgb8, w, h, &a);
            let (plane, pw, ph) = green_region(&rgb8, w, (0, 0, w, h));
            let (profile, surround) = radial_excess_and_surround(&plane, pw, ph, &a.stars);
            line(
                &format!("dial {:>3.0}% coarse {:.2}", dial * 100.0, settings.denoise.coarse_denoise()),
                sky,
                &obj,
                star,
                star_chroma(&rgb8, w, h, &a),
            ...
            println!(
                "{:<26} {}  surround {surround:+.2}  render {:.0} ms",
                "",
...
/// Two rendered PNGs of the same target, measured against one set of anchors.
///
/// The application's own output rather than a re-render: when an observer reports that a
/// revision changed something, the thing to measure is the file they were looking at.
/// `PNG_A` and `PNG_B` are the two files; anchors come from `PNG_A`.
#[test]
#[serial]
#[ignore = "diagnostic - set PNG_A and PNG_B; run with --ignored"]
fn compare_two_rendered_pngs() {
    let (Ok(a_path), Ok(b_path)) = (std::env::var("PNG_A"), std::env::var("PNG_B")) else {
        println!("set PNG_A and PNG_B");
        return;
    };
    let load = |path: &str| -> (Vec<u8>, usize, usize) {
        let img = image::open(path).unwrap().to_rgb8();
        let (w, h) = (img.width() as usize, img.height() as usize);
        (img.into_raw(), w, h)
    };
    let (a_rgb, aw, ah) = load(&a_path);
    let (b_rgb, bw, bh) = load(&b_path);
    assert_eq!((aw, ah), (bw, bh), "the two renders must be the same size");

    let (a_plane, pw, ph) = green_region(&a_rgb, aw, (0, 0, aw, ah));
    let (object, sky_box) = locate_object(&a_plane, pw, ph);
    let stars = find_isolated_stars(&a_plane, pw, ph, 25);
    let anchors = Anchors { object, sky_box, stars };
    println!("\\nanchored on {aw}x{ah}: object {object:?}, sky {sky_box:?}, {} stars", anchors.stars.len());
    header_line();
    println!("{:<26} {}", "", profile_header());

    for (label, rgb8) in [("A", &a_rgb), ("B", &b_rgb)] {
        let (sky, obj, star, bands) = measure(rgb8, aw, ah, &anchors);
        let (plane, _, _) = green_region(rgb8, aw, (0, 0, aw, ah));
        let (profile, surround) = radial_excess_and_surround(&plane, pw, ph, &anchors.stars);
        line(label, sky, &obj, star, star_chroma(rgb8, aw, ah, &anchors), &bands);
        println!("{:<26} {}  surround {surround:+.2}", "", format_profile(&profile));
    }
}

/// The saved stacks rendered at the observer's own settings.
///
/// `GRAIN_DIAL` and `LUMA_STRENGTH` default to what `settings.json` carried on
/// 2026-09-19 (0.15 and 0.15), so a before/after of a code change is measured at the
/// position the report came from rather than at the product default.
#[test]
#[serial]
#[ignore = "diagnostic - reads saved stacks; run with --ignored"]
fn render_at_observer_settings() {
    use night_amplifier::server::state::{CaptureSettings, DenoiseSettings};
    let env = |name: &str, fallback: f32| -> f32 {
        std::env::var(name).ok().and_then(|v| v.parse().ok()).unwrap_or(fallback)
    };
    let (dial, strength) = (env("GRAIN_DIAL", 0.15), env("LUMA_STRENGTH", 0.15));
    ...
    for (label, depth, stack) in stacked_sessions() {
        ...
        let d = &settings.denoise;
        println!(
            "\n=== {label}, N={depth} — dial {:.0}% p{:.2} split {:.4} coarse {:.2}, strength {:.0}% ===",
            dial * 100.0,
            d.star_protection(),
            d.grain_split(),
            d.coarse_denoise(),
        ...
        let (rgb8, w, h) = render(stack.clone(), &settings, true, stream(), depth);
        let (sky, obj, star, bands) = measure(&rgb8, w, h, &a);
        line("observer settings", sky, &obj, star, star_chroma(&rgb8, w, h, &a), &bands);
        let (plane, pw, ph) = green_region(&rgb8, w, (0, 0, w, h));
        let (profile, surround) = radial_excess_and_surround(&plane, pw, ph, &a.stars);
        println!("{:<26} {}", "", profile_header());
        println!("{:<26} {}  surround {surround:+.2}", "", format_profile(&profile));
...
/// The Background Grain dial's mapping, printed at 5 % steps.
///
/// Read off `DenoiseSettings` rather than restated, so it cannot drift from the code it
/// documents.
#[test]
#[serial]
#[ignore = "diagnostic - prints the dial's mapping; run with --ignored"]
fn print_the_dial_mapping() {
    use night_amplifier::render::LumaDenoiseConfig;
    use night_amplifier::server::state::DenoiseSettings;

    println!(
        "\n dial | star prot | grain split | coarse | wavelet k (levels 1-6)      | what moves"
    );
    println!("------+-----------+-------------+--------+-----------------------------+-----------");
    for step in 0..=20 {
        let dial = step as f32 / 20.0;
        let d = DenoiseSettings { background_grain: dial, ..Default::default() };
        let (p, split, coarse) = (d.star_protection(), d.grain_split(), d.coarse_denoise());
        let k = LumaDenoiseConfig::thresholds_for(p, coarse);
        let moves = if dial < 0.25 {
            "finest scale only (free)"
        } else if dial < 0.5 {
            "finest scale + tone curve"
        } else if dial == 0.5 {
            "<- default, = pre-dial render"
        } else {
            "coarse levels 5-6"
        };
        println!(
            "{:>4.0}% |    {p:>5.2}  |   {split:>7.4}   |  {coarse:>4.2}  | [{}] | {moves}",
            dial * 100.0,
            k.iter().map(|v| format!("{v:.2}")).collect::<Vec<_>>().join(" "),
        );
    }
}

// ---------------------------------------------------------------------------
// Star Fields research
// ---------------------------------------------------------------------------

/// What the Star Fields profile is actually trying to maximise.
///
/// Not target brightness: this mode exists to show a field of stars, and nebulosity is
/// explicitly expendable. So the score is "how many stars stand clear of how clean a
/// background", plus how *flat* what is left looks — a globular's glow rendering as a
/// concentrated patch is a complaint in its own right.
struct StarFieldScore {
    sky: f64,
    faint: f64,
    bright: f64,
    peak: f64,
    ...
    patches: f64,
    glow_ratio: f64,
    /// Deepest point of the radial profile between r=3 and r=15 px.
    ///
    /// **The one that stops this turning into oversharpening.** Amplifying the fine
    /// scales digs a dark moat around every bright star long before any of the counting
    /// metrics complain — at gain 3 the score table reported +119 % stars and a flatter
    /// glow while the picture had a black ring around every star and a grey background.
    /// A counting metric cannot see a ring; this can.
    moat: f64,
    surround: f64,
}

fn star_field_score(rgb8: &[u8], w: usize, h: usize, a: &Anchors) -> StarFieldScore {
    let (plane, pw, ph) = green_region(rgb8, w, (0, 0, w, h));
    let (sky_plane, _, _) = green_region(rgb8, w, a.sky_box);
    let sky = plane_median(&sky_plane);
    let (grain_plane, gw, gh) = green_region(rgb8, w, grain_box(a, w, h));
    let bands = octave_band_sigma(&grain_plane, gw, gh);
    let (speckle, patches) = split_bands(&bands);
    let obj = object_excess(&plane, pw, ph, a.object, 96, sky);
    let (profile, surround) = radial_excess_and_surround(&plane, pw, ph, &a.stars);
    let moat = PROFILE_RADII
        .iter()
        .zip(&profile)
        .filter(|(&r, _)| (3..=15).contains(&r))
        .map(|(_, &v)| v)
        .fold(f64::MAX, f64::min);
    StarFieldScore {
        sky,
        faint: star_count(&plane, pw, ph, 20.0),
        bright: star_count(&plane, pw, ph, 60.0),
        peak: profile[0],
        speckle,
        patches,
        moat,
        surround,
        // How concentrated the diffuse glow is. A cluster that reads as one bright patch
        // has a core far above its own outskirts; spreading the glow lowers this.
        glow_ratio: obj.core / obj.outer.max(1.0),
    }
}

fn score_header() {
    println!(
        "{:<22} {:>4} {:>7} {:>7} {:>6} {:>8} {:>8} {:>6} {:>6} {:>8}",
        "", "sky", "faint", "bright", "peak", "speckle", "patches", "glow", "moat", "surround"
    );
}

fn score_line(label: &str, s: &StarFieldScore) {
    println!(
        "{label:<22} {:>4.0} {:>7.0} {:>7.0} {:>6.0} {:>8.2} {:>8.2} {:>6.1} {:>6.1} {:>+8.2}",
        s.sky, s.faint, s.bright, s.peak, s.speckle, s.patches, s.glow_ratio, s.moat, s.surround
    );
}

/// Candidate Star Fields ladders: `(label, thresholds, gains)`, both finest-first.
///
/// One list, so the scored table and the saved crops can never drift apart — a variant
/// that reads well in the numbers and badly on screen is the whole reason to look.
type Ladder = (&'static str, [f32; 6], [f32; 6]);

fn star_field_ladders() -> Vec<Ladder> {
    // The coarse thresholds stay near the shipped ones. Crushing them (3 sigmas at every
    // level, 5-6 switched on) scored *better* on every number — and put visible dark
    // contour worms across Orion's background, because zeroing a smooth gradient's detail
    // coefficients leaves the reconstruction piecewise flat. The Orion win turned out to
    // come from the *finest* threshold anyway, which is free of that.
    const BEFORE: [f32; 6] = [1.0, 3.0, 2.0, 1.0, 0.0, 0.0];
    // What Star Fields ships with: the finest threshold boosted by
    // `STAR_FIELD_FINE_BOOST` (1.0 * 2.5 at the dial's default) and `STAR_FIELD_GAIN` on
    // the scales a star lives at.
    const NOW: [f32; 6] = [2.5, 3.0, 2.0, 1.0, 0.0, 0.0];
    vec![
        ("a-before", BEFORE, [1.0; 6]),
        ("b-now", NOW, [1.1, 1.1, 1.05, 1.0, 1.0, 1.0]),
        ("c-boost-only", NOW, [1.0; 6]),
        ("d-gain-1.25", NOW, [1.25, 1.25, 1.125, 1.0, 1.0, 1.0]),
    ]
}

/// **Star Fields research.** Threshold ladders for the wavelet, on the two sessions the
/// mode is for.
///
/// The idea under test: the shipped ladder gets *weaker* with scale, because for every
/// other mode the coarse scales are where faint nebulosity lives and must be protected.
/// Star Fields wants the opposite — stars are fine-scale, and everything coarse is either
/// sky mottle, a gradient, or a glow the observer would rather see flattened. So the
/// ladder should be inverted: protect the fine scales, crush the coarse ones.
#[test]
#[serial]
#[ignore = "diagnostic - reads saved stacks; run with --ignored"]
fn experiment_star_field_ladders() {
    use night_amplifier::render::{LumaDenoiseConfig, StretchAggressiveness, MAX_WAVELET_LEVELS};
    use night_amplifier::server::state::{CaptureSettings, DenoiseSettings};

    // Fine-protecting, coarse-crushing ladders against the shipped one. `k[i]` is in
    // sigmas of level `i+1`'s own measured noise.
    const CRUSH: [f32; MAX_WAVELET_LEVELS] = [2.0, 3.0, 3.0, 3.0, 3.0, 3.0];
    let ladders = star_field_ladders();

    for (label, depth, stack) in stacked_sessions() {
        println!("\n=== {label}, N={depth} — Star Fields ===");
        let a = anchors(&format!("saved-{label}"), &stack, depth);
        score_header();
        for (name, k, gain) in &ladders {
            let mut settings = CaptureSettings::default();
            settings.stretch_aggressiveness = StretchAggressiveness::Low;
            settings.denoise = DenoiseSettings {
                chroma: true,
                chroma_strength: 1.0,
            ...
            let (rgb8, w, h) = render_with(
                ...
                depth,
                move |config| {
                    config.denoise.luma = LumaDenoiseConfig {
                        enabled: true,
                        k,
                        gain,
                        strength: 1.0,
            ...
            score_line(name, &star_field_score(&rgb8, w, h, &a));
        }
    }
}

/// Saves crops of the same field under several ladders, for looking at rather than
/// scoring. `CROP_OUT` is the directory.
#[test]
#[serial]
#[ignore = "diagnostic - reads saved stacks; run with --ignored"]
fn save_star_field_crops() {
    use night_amplifier::render::{LumaDenoiseConfig, StretchAggressiveness};
    use night_amplifier::server::state::{CaptureSettings, DenoiseSettings};

    let out = std::env::var("CROP_OUT").unwrap_or_else(|_| "/tmp/crops".into());
    std::fs::create_dir_all(&out).unwrap();
    let span: usize = std::env::var("CROP_SPAN").ok().and_then(|v| v.parse().ok()).unwrap_or(420);
    ...
    for (label, depth, stack) in stacked_sessions() {
        ...
        for (name, k, gain) in &ladders {
            ...
            let (rgb8, w, h) = render_with(
                ...
                move |config| {
                    config.denoise.luma =
                        LumaDenoiseConfig { enabled: true, k, gain, strength: 1.0 };
                },
            );
            let (cx, cy) = a.object;
            let x0 = cx.saturating_sub(span / 2).min(w.saturating_sub(span));
            let y0 = cy.saturating_sub(span / 2).min(h.saturating_sub(span));
            let mut crop = Vec::with_capacity(span * span * 3);
            for y in y0..y0 + span.min(h) {
                let row = (y * w + x0) * 3;
                crop.extend_from_slice(&rgb8[row..row + span.min(w) * 3]);
            }
            let path = format!("{out}/{label}_{name}.png");
            image::save_buffer(
                &path,
                &crop,
                span.min(w) as u32,
