...
fn experiment_star_field_ladders() {
    ...
    // `k[i]` is in sigmas of level `i+1`'s own measured noise. The coarse-crushing
    // ladder this experiment started from is gone rather than kept as a dead constant —
    // `star_field_ladders` says why it was rejected, and the list there is the one thing
    // the scores and the saved crops both read.
