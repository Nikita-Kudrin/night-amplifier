...
fn test_alignment() {
    ...
    assert!(
        (dx - (-5.0)).abs() < 2.0,
        "X offset should be ~-5, got {}",
        dx
    );
    assert!(
        (dy - (-3.0)).abs() < 2.0,
        "Y offset should be ~-3, got {}",
        dy
    );
}
...
fn test_alignment_with_planetary_disk() {
    ...
    assert!(
        (dx - (-3.0)).abs() < 1.5,
        "X offset should be ~-3, got {}",
        dx
    );
    assert!((dy - 2.0).abs() < 1.5, "Y offset should be ~2, got {}", dy);
}
...
fn test_alignment_centroid_fallback() {
    ...
    info!(
        "Centroid fallback test results: dx={}, dy={}, ncc={}",
        dx, dy, ncc
    );
    ...
    assert!(
        (dx - (-10.0)).abs() < 1.0,
        "X offset should be ~-10, got {}",
        dx
    );
    assert!(
        (dy - (-5.0)).abs() < 1.0,
        "Y offset should be ~-5, got {}",
        dy
    );
}
