...
use crate::statistics::select_median;
...
/// Samples drawn per level to estimate that level's noise, and to read the coarse
/// levels' mask off the smoothed plane.
///
/// Read by [`crate::statistics::select_median`], **not** `fast_median`, which
/// `par_sort_unstable`s anything from 4096 up. Since the sigma became per-level this is
/// drawn twice a level — up to twelve times a frame — so those were a dozen parallel
/// full sorts inside one pass. Dropping them and halving the count twice takes the
/// 1440² pass from 14.2 ms a frame to 10.6 with the coarse levels off and from 21.0 to
/// 13.8 with them on, and leaves the measured thresholds and star profiles unchanged to
/// 0.1 output levels. The same trap `black_point::CENTRE_MAX_SAMPLES` documents.
///
/// 16 384 rather than more: the gather is a strided read over two full 8 MB planes, so
/// its cost falls with the sample count while the sigma it produces is already placed to
/// ~1 % of itself. Swept at 1440², one bench group at a time (per call, luma / luma with
/// coarse levels): 8192 reads 10.6/13.4 ms, 16 384 10.6/13.8, 32 768 11.0/14.9, 65 536
/// 12.8/17.7, 131 072 14.0/19.7. The bottom is flat between 8192 and 32 768 — run-to-run
/// drift is ~3 % — so this takes the middle of it and keeps the tighter sigma.
const MAX_SIGMA_SAMPLES: usize = 1 << 14;
...
fn robust_level(plane: &[f32], n: usize) -> (f32, f32) {
    ...
    let spread = select_median(&mut samples) * MAD_TO_SIGMA;
...
/// Robust sigma of a detail plane, from a strided subsample.
///
/// `select_median`, not `statistics::fast_median`: this runs once per level rather than
/// once per frame now, and a parallel full sort per call is what made that expensive.
/// See [`MAX_SIGMA_SAMPLES`].
fn estimate_detail_sigma(coarse: &[f32], next: &[f32], n: usize) -> f32 {
    ...
    let sigma = select_median(&mut samples) * MAD_TO_SIGMA;
