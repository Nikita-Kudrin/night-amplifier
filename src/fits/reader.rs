//! FITS image reading.
//!
//! Lives next to the writers rather than under `camera::simulated::loaders`, which is
//! where it used to sit. That placement had two costs: a FITS reader is not a
//! simulated-camera concern, and the integration harness — a separate crate — could not
//! reach it, so it carried a second implementation that had to interleave a planar FITS
//! before handing it to `Frame::from_raw`, reinstating exactly the double conversion
//! this reader was written to remove.
...
use super::{FitsColourLayout, FitsShape};
use crate::error::{Result, StackError};
use crate::ffi_safety::catch_ffi_panic;
use crate::Frame;
use rayon::prelude::*;

/// Reads the first image HDU of a FITS file into a planar [`Frame`].
pub fn read_frame(path: &Path) -> Result<Frame> {
    let path_str = path
        .to_str()
        .ok_or_else(|| StackError::FitsRead("Invalid path".to_string()))?;

    let path_owned = path_str.to_string();
    let mut fitsfile = catch_ffi_panic("cfitsio::open", || FitsFile::open(&path_owned))
        .map_err(StackError::from)?
        .map_err(|e| StackError::FitsRead(format!("Failed to open FITS: {}", e)))?;

    let mut valid_hdu_idx = None;
    for hdu_idx in 0..fitsfile
        .num_hdus()
        .map_err(|e| StackError::FitsRead(format!("Failed to get HDU count: {}", e)))?
    {
    ...
    let hdu_idx = valid_hdu_idx.ok_or_else(|| {
        StackError::FitsRead("FITS file does not contain image data in any HDU".to_string())
    })?;

    let hdu = fitsfile
        .hdu(hdu_idx)
        .map_err(|e| StackError::FitsRead(format!("Failed to re-open HDU {}: {}", hdu_idx, e)))?;
...
fn extract_fits_info(info: &HduInfo) -> Result<(FitsShape, ImageType)> {
    match info {
        HduInfo::ImageInfo { shape, image_type } => {
            let parsed = super::interpret_shape(shape).ok_or_else(|| {
                StackError::FitsRead(format!("Unsupported FITS shape: {:?}", shape))
            })?;
            Ok((parsed, *image_type))
        }
        _ => Err(StackError::FitsRead(
...
fn read_fits_data(
    ...
    macro_rules! read {
        ($label:literal, $ty:ty) => {{
            let data: Vec<$ty> = catch_ffi_panic($label, || hdu.read_image(fitsfile))
                .map_err(StackError::from)?
                .map_err(|e| StackError::FitsRead(format!("Failed to read data: {}", e)))?;
            data
        }};
    }
    ...
    match image_type {
        ...
        _ => Err(StackError::FitsRead(format!(
...
fn planar_frame<T: Copy + Send + Sync>(
    ...
    if samples.len() < expected {
        return Err(StackError::FitsRead(format!(
    ...
    Frame::from_f32_vec(data, width, height, channels)
        .map_err(|e| StackError::FitsRead(format!("Failed to create frame: {}", e)))
}
...
mod tests {
    ...
    fn test_load_nonexistent_fits() {
        let result = read_frame(Path::new("this_file_does_not_exist_xyz.fits"));
        assert!(result.is_err());
        assert!(matches!(result.unwrap_err(), StackError::FitsRead(_)));
    }
